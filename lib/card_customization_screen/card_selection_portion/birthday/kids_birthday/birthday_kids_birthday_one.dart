...
class BirthdayKidsBirthdayOne extends StatelessWidget {
  const BirthdayKidsBirthdayOne({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    formalInvitationHeaderTextPartTwo = 'to a';
      ...
      children: [
        ...
        Selector<CardCustomizationScreenChangeNotifier, bool>(
            ...
            builder: (context, value, child) {
              ...
              return Visibility(visible: animatedTextContainerVisibilityList[1], maintainState: true, maintainSize: true, maintainAnimation: true, child: invitationCardSingleTextContainer(text: '${appendDateSuffix(date: int.parse((textFieldDataMap[hintTextList[1]]!.replaceRange(textFieldDataMap[hintTextList[1]]!.indexOf(' '), textFieldDataMap[hintTextList[1]]!.length, '').trim())))}\nbirthday\nparty!\nfor'.toUpperCase(), animationTextNumber: containerIndexAnimationNumber[1], myContainerIndex: 1));
            }),
        ...
        Selector<CardCustomizationScreenChangeNotifier, bool>(
            ...
            builder: (context, value, child) {
              ...
              return Visibility(visible: animatedTextContainerVisibilityList[3], maintainState: true, maintainSize: true, maintainAnimation: true, child: Container(alignment: Alignment.centerRight, padding:const EdgeInsets.only(right: 32.0) , child: invitationCardSingleTextContainer(text: '${DateFormat('EEEE').format(pickedDate!)},\n${pickedDate.getMonthName(monthNumber: pickedDate!.month)} ${appendDateSuffix(date: int.parse(DateFormat('d').format(pickedDate!)))}, ${(DateFormat('yyyy').format(pickedDate!))}\nat ${pickedTime!.format(context).toLowerCase()} in ${reformatAddress(address: textFieldDataMap[hintTextList[2]]!)}', animationTextNumber: containerIndexAnimationNumber[3], myContainerIndex: 3)));
            }),
        ...
        Selector<CardCustomizationScreenChangeNotifier, bool>(
            ...
            builder: (context, value, child) {
              ...
              return Visibility(visible: animatedTextContainerVisibilityList[4], maintainState: true, maintainSize: true, maintainAnimation: true, child: Container(alignment: Alignment.centerRight, padding:const EdgeInsets.only(right: 45.0), child: invitationCardSingleTextContainer(text: 'reception to follow at \n ${textFieldDataMap[hintTextList[3]]}', animationTextNumber: containerIndexAnimationNumber[4], myContainerIndex: 4)));
            }),
